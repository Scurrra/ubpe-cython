...
UbpeClassic<DocType, TokenType>::UbpeClassic(uint32_t n_tokens,
                                             uint32_t alphabet_size)
    : UbpeBase<DocType, TokenType>(n_tokens, alphabet_size) {}

template <DocumentT DocType, typename TokenType>
UbpeClassic<DocType, TokenType>::UbpeClassic(
...
UbpeClassic<DocType, TokenType>::UbpeClassic(
    ...
    std::transform(tokens_forward_mapper.cbegin(), tokens_forward_mapper.cend(),
                   std::back_inserter(this->pairs),
                   [](const auto& element) { return element.first; });
}

template <DocumentT DocType, typename TokenType>
void UbpeClassic<DocType, TokenType>::fit(const std::vector<DocType>& corpus,
    ...
    std::transform(corpus.cbegin(), corpus.cend(), std::back_inserter(_corpus),
    ...
    while (max_token < this->n_tokens) {
        ...
        std::for_each(_corpus.begin(), _corpus.end(), [this, &sub](auto& doc) {
    ...
    std::transform(this->tokens_forward_mapper.cbegin(),
                   this->tokens_forward_mapper.cend(),
                   std::back_inserter(this->pairs),
                   [](const auto& element) { return element.first; });
}
...
UbpeClassic<DocType, TokenType>::encode(const DocType& doc, uint8_t) const {
    ...
    while (true) {
        ...
        std::transform(
            _doc.cbegin(), _doc.cend() - 1, _doc.cbegin() + 1,
            std::inserter(pairs, pairs.end()),
            [](const auto& left, const auto& right) -> std::vector<uint32_t> {
                return {left, right};
            });
...
template <DocumentT DocType, typename TokenType>
DocType UbpeClassic<DocType, TokenType>::decode(
    ...
    for (size_t ti = 0, di = 0; ti < tokens.size(); ti++) {
        ...
        while (di < document.size()) {
            ...
            if (this->tokens_backward_mapper.contains(document[di])) {
                ...
                auto expander = this->tokens_backward_mapper.at(document[di]);
                ...
                // `di` is not changed, so it point's to the first token in the
                // substitution pair and it will be checked at the next
                // iteration
            } else {
                // `document[di]` is in alphabet as it can not be substituted
...
UbpeClassic<DocType, TokenType>::getBackwardMapper() const {
    return this->tokens_backward_mapper;
}

template <DocumentT DocType, typename TokenType>
std::map<uint32_t, float> UbpeClassic<DocType, TokenType>::getTokensWeights()
    const {
    return this->tokens_weights;
}

template <DocumentT DocType, typename TokenType>
std::map<TokenType, uint32_t> UbpeClassic<DocType, TokenType>::getAlphabet()
    const {
